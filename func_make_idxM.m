function idxM = func_make_idxM(rawData, kSize)
    ...
    Njump = 96;% 2; % 12; % 24; % 44;% 64; %Njump = 48; %Njump = 24;% Njump = 256;
    % Njump = 3;
    for iter = 1 : Njump : numel(uqv)

        tic
        %     M1 = sparse(   Ael(:).'== uqv( iter : iter + Njump-1 )   );
        % M1 = (   Ael(:).'== uqv( iter : iter + Njump-1 )   ); % fadilali og line
        if iter + Njump-1  < length(uqv)
        ...
        for kiter = 1 : prod(kSize) %iterate through idxM's columns.
            ...
            idx_clm = sum(cumprod(~M1,2),2)+1;
            % idxM(iter : iter + Njump-1,kiter) = idx_clm; % fadilali og
            if length(iter : iter + Njump-1) == length(idx_clm)
                idxM(iter : iter + Njump-1,kiter) = idx_clm;
            else
                idxM(iter : iter + length(idx_clm)-1,kiter) = idx_clm;
            end
    ...
    idxMDir = '/idxM/';
    save(...
        strcat(idxMDir,'idxM_',num2str(Nx),'Nx',num2str(Ny),'Ny',num2str(Nz),'Nz_',num2str(Nkx),'Nkx',num2str(Nky),'Nky',num2str(Nkz),'Nkz.mat'),...
        'idxM', '-v7.3');
